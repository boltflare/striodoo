...
class anexo72Report(models.AbstractModel):
	...
	def _do_query(self,options):
		...
		else (detalle.quantity) end AS quantity,
		detalle.price_unit,
		...
		where (detalle.date BETWEEN '{}' AND '{}') detalle.invoice_id = invoice.id and invoice.partner_id = cliente.id and detalle.account_id = cuenta.id and invoice.user_id = sale.id
