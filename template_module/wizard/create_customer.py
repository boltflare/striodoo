...
class CreateCustomer(models.TransientModel):
	...
	def create_customer(self):
		...
		for record in self.env['muki.rest'].browse(active_ids):
			self.env["res.partner"].create({'name':record.nombre,'visitor':record.hvisit,'street':record.hstreet,'street2':record.hstreet2,'city':record.hcity,'zip':record.hzip,'visitor_categ':record.hcateg ,'email':record.visitor_email})
