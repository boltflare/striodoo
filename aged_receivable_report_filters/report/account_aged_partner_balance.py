...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        """Override this method to filter account move lines based on analytic accounts."""
        ...
        """Filtered account move line based on analytic account ids. """
        if ctx.get('analtic_account_ids'):
            query += ''' AND l.analytic_account_id in %s'''
            arg_list += (tuple(ctx.get('analtic_account_ids')),)
        ...
        for i in range(5):
            ...
            if ctx.get('analtic_account_ids'):
                query += ''' AND l.analytic_account_id in %s'''
                args_list += (tuple(ctx.get('analtic_account_ids')),)
            query += ''' ORDER BY COALESCE(l.date_maturity, l.date)'''
        ...
        undue_amounts = {}
        arg_not_due = (tuple(move_state), tuple(account_type), date_from, tuple(partner_ids), date_from, tuple(company_ids))
        query = '''SELECT l.id
                FROM account_move_line AS l, account_account, account_move am
                WHERE (l.account_id = account_account.id) AND (l.move_id = am.id)
                    AND (am.state IN %s)
        ...
        if ctx.get('analtic_account_ids'):
            query += ''' AND l.analytic_account_id in %s'''
            arg_not_due += (tuple(ctx.get('analtic_account_ids')),)
        query += ''' ORDER BY COALESCE(l.date_maturity, l.date)'''
        cr.execute(query, arg_not_due)
        aml_ids = cr.fetchall()
        aml_ids = aml_ids and [x[0] for x in aml_ids] or []
